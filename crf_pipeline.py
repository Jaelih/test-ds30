...
# Train CRF model (uses spaCy's built-in BILUO encoding)
python crf_pipeline.py --action train-crf --train corpus/train.spacy --dev corpus/dev.spacy --output training/crf

# Train CRF with custom hyperparameters
python crf_pipeline.py --action train-crf --train corpus/train.spacy --dev corpus/dev.spacy --output training/crf --dropout 0.5
...
try:
    ...
    msg.warn("PyTorch not available. CRF training will be disabled.")
...
if TORCH_AVAILABLE:
    ...
    class CRFModel(nn.Module):
        """CRF-only model for sequence labeling (no BiLSTM layer)."""
        ...
        def __init__(self, input_dim: int, num_labels: int, dropout: float = 0.3):
            ...
            # Emission layer (maps tok2vec embeddings directly to label scores)
        ...
        def forward(self, embeddings, labels=None, mask=None):
            ...
            Args:
                embeddings: (batch_size, seq_len, input_dim) - tok2vec embeddings
            ...
            """
            # Apply dropout and compute emissions
            embeddings = self.dropout(embeddings)
            emissions = self.hidden2tag(embeddings)  # (batch_size, seq_len, num_labels)
            ...
            # Inference mode: decode best path using Viterbi algorithm
            if mask is None:
    ...
    class CRFNER(TrainablePipe):
        """
        spaCy pipeline component for CRF-only NER.
        Uses spaCy's built-in BILUO encoding utilities.
        """
        
        def __init__(self, vocab, model, name="crf_ner", *, tok2vec=None, labels=None):
            self.vocab = vocab
            self.model = model
            self.name = name
            self.cfg = {}
        ...
        def from_disk(self, path, *, exclude=tuple()):
            ...
            msg.info(f"Recreating model from disk: num_labels={num_labels}, input_dim={input_dim}")
            
            # Create new model with correct num_labels
            pytorch_model = CRFModel(
                input_dim=input_dim,
                num_labels=num_labels,
                dropout=dropout
            )
    ...
    def build_crf_model(input_dim, dropout=0.3, num_labels=13):
        """Build a simple PyTorchWrapper model for CRF."""
    ...
    def make_crf_ner(nlp, name, dropout, labels):
        """Factory function to create CRF NER component."""
        ...
        model = build_crf_model(input_dim, dropout, num_labels)
        
        # Create component
        return CRFNER(nlp.vocab, model, name=name, tok2vec=tok2vec, labels=entity_labels)
    ...
    def make_crf_ner_trf(nlp, name, dropout, labels, tok2vec):
        """
        Factory function to create CRF NER component with TransformerListener.
        ...
        # Create the PyTorch CRF model
        pytorch_crf = CRFModel(
            input_dim=input_dim,
            num_labels=num_labels,
            dropout=dropout
        )
        ...
        pytorch_wrapper = PyTorchWrapper(pytorch_crf)
        pytorch_wrapper.attrs["pytorch_model"] = pytorch_crf
        ...
        msg.info(f"Created CRF NER component with TransformerListener tok2vec (input_dim={input_dim}, num_labels={num_labels})")
...
def create_crf_config(lang: str = "tl", use_gpu: bool = False, dropout: float = 0.3) -> str:
    """
    Create a CRF-only spaCy configuration for NER training.
...
def create_transformer_config(lang: str = "tl", transformer_name: str = "xlm-roberta-base", use_gpu: bool = False, dropout: float = 0.3) -> str:
    """
    Create a transformer + CRF spaCy configuration for NER training.
...
[nlp]
lang = "{lang}"
pipeline = ["transformer","crf_ner_trf"]
batch_size = 128
disabled = []
before_creation = null
after_creation = null
...
[components.crf_ner_trf]
factory = "crf_ner_trf"
dropout = {dropout}

[components.crf_ner_trf.tok2vec]
@architectures = "spacy-transformers.TransformerListener.v1"
grad_factor = 1.0
upstream = "*"

[components.crf_ner_trf.tok2vec.pooling]
@layers = "reduce_mean.v1"
...
def train_crf(
    train_path: str = "corpus/train.spacy",
    dev_path: str = "corpus/dev.spacy",
    output_dir: str = "training/crf",
    gpu_id: int = -1,
    dropout: float = 0.3,
    config_path: Optional[str] = None
):
    """
    Train a CRF-only NER model with BILUO encoding.
    ...
    Args:
        ...
        config_path: Optional custom config file path
    """
    msg.info("Training CRF model with BILUO encoding...")
    ...
    if config_path is None:
        config_path = Path(output_dir) / "config.cfg"
        config_str = create_crf_config(
            lang="tl",
            use_gpu=(gpu_id >= 0),
            dropout=dropout
        )
        config_path.write_text(config_str)
        msg.info(f"Created CRF config at {config_path}")
...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        choices=[
            "install", "process", "train-baseline", "train-crf", "train-transformer",
    ...
    parser.add_argument("--dropout", type=float, default=0.3, help="Dropout rate for CRF")
    ...
    try:
        ...
        elif args.action == "train-crf":
            if not args.train or not args.dev:
                msg.fail("--train and --dev required for training")
                return
            if not TORCH_AVAILABLE:
                msg.fail("PyTorch is required for CRF training. Please install torch and torchcrf.")
                return
            train_crf(
                args.train, args.dev,
                args.output or "training/crf",
                args.gpu_id,
                args.dropout,
                args.config
            )
